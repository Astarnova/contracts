...
import "./interfaces/INovaPair.sol";
...
contract MasterChef is Ownable, ReentrancyGuard {
  ...
  // Percentage of pool rewards that goes to the community.
  ...
  event SetCommunityPercent(uint256 oldPercent, uint256 newPercent);
  ...
  constructor(
    ...
    require(
      _communityPercent <= 1000,
      "constructor: invalid community percent value"
    );
    require(
      _teamPercent + _treasuryPercent + _communityPercent <= 1000,
  ...
  function _updatePool(uint256 _pid) internal validatePoolByPid(_pid) {
    ...
    nova.mint(communityAddress, (novaReward * communityPercent) / total);
  ...
  function depositWithPermit(
    ...
    PoolInfo storage pool = poolInfo[pid];
    INovaPair pair = INovaPair(address(pool.lpToken));
  ...
  function safeNovaTransfer(address _to, uint256 _amount) internal {
    if (nova.balanceOf(address(this)) > totalNovaInPools) {
      //novaBal = total Nova in NovaDistributor - total Nova in Nova pools, this will make sure that NovaDistributor never transfer rewards from deposited Nova pools
  ...
  function updateEmissionRate(uint256 _novaPerSec, bool _withUpdate) public onlyOwner {
    if (_withUpdate) {
      _massUpdatePools();
    }
  ...
  function updateAllocPoint(uint256 _pid, uint256 _allocPoint, bool _withUpdate)
  public
  onlyOwner
  {
    if (_withUpdate) {
  ...
  function setTeamPercent(uint256 _newTeamPercent) public onlyOwner {
    ...
    require(
      treasuryPercent + _newTeamPercent + communityPercent <= 1000,
  ...
  function setTreasuryPercent(uint256 _newTreasuryPercent) public onlyOwner {
    ...
    require(
      teamPercent + _newTreasuryPercent + communityPercent <= 1000,
  ...
  // Update the community address by the previous community.
  function setCommunityAddress(address _communityAddress) public {
    require(
      msg.sender == communityAddress,
      "set community address: only previous community can call this method"
    );
    require(
      _communityAddress != address(0),
      "set community address: invalid new community address"
    );
    communityAddress = _communityAddress;
    emit SetCommunityAddress(msg.sender, _communityAddress);
  }

  function setCommunityPercent(uint256 _newCommunityPercent) public onlyOwner {
    require(
      _newCommunityPercent <= 1000,
      "set community percent: invalid percent value"
    );
    require(
      teamPercent + _newCommunityPercent + treasuryPercent <= 1000,
      "set community percent: total percent over max"
    );
    emit SetCommunityPercent(communityPercent, _newCommunityPercent);
    communityPercent = _newCommunityPercent;
  }
}
